...
def data_pre(min):
    ...
    # 时间序列做为标签
    if(min==1):
        dataframe = pd.read_csv(r'E:\data_all\data_wdz_all_jan_one.csv',usecols=[0,10,13],engine='python',index_col=0)
        #dataframe['Temp'][4:] = dataframe['Temp'].rolling(5).mean()[4:]
        ...
        X_train, X_test, y_train, y_test = X[:train_size], X[train_size:test_size], Y[:train_size], Y[
                                                                                                    train_size:test_size]
        X_train = X_train.reshape((-1, 1, 7))
        X_test = X_test.reshape((-1, 1, 7))
    elif (min ==10):

        #dataframe = pd.read_csv(r'E:\data_all\data_wdz_all_jan_10.csv', usecols=[0, 1, 5, 6, 7, 10, 12, 13], engine='python', index_col=0)
        dataframe = pd.read_csv(r'E:\data_all\data_wdz_all_jan_10.csv', usecols=[0, 10, 12, 13], engine='python', index_col=0)
